...
class LayerController extends Controller
{
    ...
    public function createLayer(Request $request, $id)
    {
        ...
        if ($inputs['parent'] > 0) {
            $layer = Layer::where('fileId', $id)->where('status', Layer::STATUS_NORMAL)->find($inputs['parent']);

            if (is_null($layer)) {
                return Output::error(trans('common.parent_layer_not_found'), 50002, [], Response::HTTP_BAD_REQUEST);
            }
        }
        ...
        $layer->type = Layer::getTypeByName($inputs['name']);
        $layer->fileId = $id;
        $layer->parentId = intval($inputs['parent']);
        $layer->position = 1000.1;
        $layer->data = $inputs['data'];
        $layer->styles = $inputs['styles'];
        $layer->status = Layer::STATUS_NORMAL;
        $layer->createdAt = date('Y-m-d H:i:s');

        if (!$layer->save()) {
            return Output::error(trans('common.server_is_busy'), 50003, [], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
